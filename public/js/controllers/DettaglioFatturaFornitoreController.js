...
.controller("DettaglioFatturaFornitoreController",
 ['$scope', '$uibModal','$location','$filter', '$window', '$rootScope', '$routeParams','$sessionStorage', 'FattureFornitoriService', 'FornitoreService',
 function ($scope, $uibModal, $location, $filter, $window, $rootScope, $routeParams,$sessionStorage, FattureFornitoriService, FornitoreService) {
	'use strict';
	$scope.temp = {};
	$scope.persistent = {};
...
$scope.loadDatiFattura = function () {
		...
		FattureFornitoriService.getFatturaCoreById($scope.persistent.idFattura).then(function(response) {  
	    	...
	    	if(handleResponseResult.next){
				...
				$scope.persistent.fattura.core.idFornitore = response.data.fattura.ID_FORNITORE;	
				...
				//header fattura
				$scope.loadDatiFornitore(response.data.fattura.ID_FORNITORE);			
		...
		$scope.loadDatiFornitore = function (id_fornitore) {	    
		
			FornitoreService.getFornitoreByIdDettFatt(id_fornitore).then(function(response) {  
	...
	$scope.openModalGeneraNDCR = function (pIdFattura) {		
		...
		$uibModal.open({
		templateUrl: './html/fatture-acquisto/modalGeneraNdc.html',
		scope:$scope,	
		backdrop:'static',		
